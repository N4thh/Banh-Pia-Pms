...
export default function Customer() {
    ...
    // ref trỏ trực tiếp tới panel để điều khiển transform trong lúc kéo tay
    ...
    useEffect(() => {
        if (pendingUser) {
            ...
            requestAnimationFrame(() => {
                requestAnimationFrame(() => setVisiblePendingModal(true));  //Run two times to ensure the browser is running a complete frame
            });
        } else {
   ...
   if(step === 2) return null;
    ...
    return(
        <div className="space-y-[2vh] text-[#3D2008]">
            ...
            text-[20px] sm:text-[21px] md:text-[22px] lg:text-[23px] xl:text-[24px] 2xl:text-[25px]"
            >Thông tin liên hệ</p>
            {/* input */}
            <div>
                <input
            ...
            <div className="flex flex-col">
                <button className="w-full lg:w-fit lg:ml-auto inset-0 border py-3 px-6 rounded-xl
            ...
            {renderPendingModal && (
                <div
                    className={`fixed inset-0 z-50 flex items-center justify-center max-lg:items-end
                        bg-black/50 transition-opacity duration-300
                        ${visiblePendingModal ? "opacity-100" : "opacity-0"}`}
                >
                    <div
                        ref={panelRef}
                        className={`bg-[#E6F2F8] rounded-2xl p-6 w-[90vw] max-w-md text-[#3D2008] flex flex-col gap-4
                            max-lg:rounded-t-2xl! max-lg:rounded-b-none! max-lg:w-full! max-lg:max-w-none! max-lg:mb-0!
                            max-lg:transition-transform max-lg:duration-300 max-lg:ease-[cubic-bezier(0.32,0.72,0,1)]
                            ${visiblePendingModal ? "max-lg:translate-y-0" : "max-lg:translate-y-full"}`}
                    >
                        {/* Drag handle - chỉ hiện trên mobile, kéo được để đóng */}
                        <div
                            className="hidden max-lg:flex justify-center -mt-2 -mb-2 pb-2 pt-1
                        ...
                        <div className="flex flex-col text-[#0088FF]">
                            <h3 className="font-vollkorn font-semibold
                                text-[16px] sm:text-[17px] md:text-[18px] lg:text-[19px] xl:text-[20px] 2xl:text-[21px]">
                                Chào mừng quay trở lại, {pendingUser?.fullName}!
                            </h3>
                            <span className="text-[14px] sm:text-[14px] md:text-[14px] lg:text-[15px] xl:text-[15px] 2xl:text-[167px]"
                            >Bạn có muốn hệ thống tự động điền thông tin còn lại?</span>
                        </div>

                        {pendingUser?.latestAddress && (
                            <div className="px-3 py-2 border border-[#0088FF]/25 rounded-lg text-[#0088FF]/65
                            text-[12px] sm:text-[13px] md:text-[14px] lg:text-[15px] xl:text-[16px]">
                                <p className="lg:font-medium text-[14px] sm:text-[14px] md:text-[14px] lg:text-[15px] xl:text-[15px] 2xl:text-[15px]">Địa chỉ gần nhất của bạn:</p>
                                <p className="py-[0.5vh] text-[12px] sm:text-[12px] md:text-[12px] lg:text-[13px] xl:text-[13px] 2xl:text-[13px]">
                        ...
                        <div className="flex justify-between text-[#0088FF] text-[16px]">
                            <button
                            onClick={handleSkipFill}
                            className="px-2 font-semibold hover:text-[#0088FF]/65active:text-[#0088FF]/20 transition-colors
